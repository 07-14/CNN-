...
class DenseNet(tf.keras.Model):
  ...
  def __init__(self, depth_of_model, growth_rate, num_of_blocks,
    ...
    self.depth_of_model = depth_of_model  # 网络的总深度，包括了dense blocks所占部分和其它部分
    self.growth_rate = growth_rate  # 每一层所增加的特征图的数目
    self.num_of_blocks = num_of_blocks  # dense block数目
    self.output_classes = output_classes  # 输出数目
    self.num_layers_in_each_block = num_layers_in_each_block  # 每个dense block中的层数
    self.data_format = data_format  # 数据的组织格式?
    self.bottleneck = bottleneck  # 在dense layers之间的3 * 3卷积之前是否作1 * 1卷积来减少特征图的数量
    self.compression = compression  # 是否加入transition layer(仍然采用1 * 1卷积和2 * 2池化)?
    ...
    if isinstance(self.num_layers_in_each_block, list) or isinstance(
      ...
      self.num_layers_in_each_block = list(self.num_layers_in_each_block)  # 如果以列表或元组形式传入每一个dense block包含的层数，则全部转换为列表?
    else:
      if self.num_layers_in_each_block == -1:
        if self.num_of_blocks != 3:
          raise ValueError(
              "Number of blocks must be 3 if num_layers_in_each_block is -1")  # 如果没有设定每一个dense block中的层数，dense block的数目必须为3？
        if (self.depth_of_model - 4) % 3 == 0:
          num_layers = (self.depth_of_model - 4) / 3  # 每一个dense block中的层数相等
          if self.bottleneck:
            num_layers //= 2  # //整除 为何加入bottleneck后每一个dense block中的层数会减半
          self.num_layers_in_each_block = [num_layers] * self.num_of_blocks  # 将每一个dense block中的层数扩展为列表[num_layers, num_layers, ...]
        else:
          raise ValueError("Depth must be 3N+4 if num_layer_in_each_block=-1")
      else:
        self.num_layers_in_each_block = [
            self.num_layers_in_each_block] * self.num_of_blocks  # 同上将每一个dense block中的层数扩展为列表[num_layers, num_layers, ...]

    axis = -1 if self.data_format == "channels_last" else 1

    # setting the filters and stride of the initial covn layer.
    if self.pool_initial:
      init_filters = (7, 7)  # 在最开始的卷积层先做7 * 7的卷积再做3 * 3的池化
      stride = (2, 2)
    else:
      init_filters = (3, 3)  # 在最开始的卷积层上直接采用3 * 3的卷积
      stride = (1, 1)

    self.num_filters = 2 * self.growth_rate  # num_filters是指谁的？
    ...
    # 如果include_top为false的话，是否就退变为了一个feature extractor；即只用加载后top之前的层即可
    if self.include_top:
    ...
    num_filters_after_each_block = [self.num_filters]  # filter限定之后，相当于把feature map的数目也限定了，即每一个dense block出来后特征图的数目必须相等
    for i in range(1, self.num_of_blocks):
      ...
      num_filters_after_each_block.append(temp_num_filters)  # 可以看出，前面的num_filters确实是一个标量，但在这个循环中，基于num_filters计算得到每一个dense block出来的特征图的数目
    ...
    for i in range(self.num_of_blocks):
      self.dense_blocks.append(DenseBlock(self.num_layers_in_each_block[i],
                                          ...
                                          self.weight_decay,
                                          self.dropout_rate))
      if i+1 < self.num_of_blocks:  # 除去最后一个block，其它都添加TransitionBlock在后面
        self.transition_blocks.append(
            TransitionBlock(num_filters_after_each_block[i+1],
                            ...
                            self.dropout_rate))
  """ 总结
  1. 初始化每一个dense block中的层数，保存在列表num_layers_in_each_block中；
  2. 初始化第一个dense block之前的卷积层；
  3. 计算每一个dense block完成后并经过transition后输出的特征图的数目，保存在
     num_filters_after_each_block中
  4. 根据num_layers_in_each_block列表构建所有的dense blocks，根据
     num_filters_after_each_block构建所有的transition blocks
  5. 其它初始化了两个BN层  
  """
